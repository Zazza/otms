...
class Ajax extends Main {
	...
	public function files() {
		...
		if ($dh  = opendir($path)) {
    		while (false !== ($filename = readdir($dh))) {
    			if ( ($filename != ".") and ($filename != "..") and ($filename != "_thumb") and ($filename != ".htaccess") ) {
	...
	public function getTotalSize() {
		...
		while ($sFileName=readdir($dDir)) {
			if ( ($sFileName != '.') and ($sFileName != '..') and ($sFileName != "_thumb") and ($sFileName != ".htaccess") ) {
    ...
    public function moveFiles($params) {
        ...
        foreach($params["file"] as $part) {
            ...
            if (file_exists($this->abs_thumbDir . $curdir . $part)) {
