...
class Model_Ui extends Engine_Model {
	...
	public function getInfo($loginSession) {
		...
		if (!$this->memcached->load()) {
			...
			if (count($data) == 1) {
				...
				$data[0]["skin"] = $this->getSkin($loginSession["id"]);
	...
	public function setSkin($skin) {
		if (count($this->getSkin($this->registry["ui"]["id"])) > 0) {
			$sql = "UPDATE users_settings SET skin = :skin WHERE uid = :uid LIMIT 1";
			
			$res = $this->registry['db']->prepare($sql);
			$param = array(":uid" => $this->registry["ui"]["id"], ":skin" => $skin);
			$res->execute($param);
		} else {
			$sql = "INSERT INTO users_settings (uid, skin) VALUES (:uid, :skin)";
	...
	private function getSkin($uid) {
		$sql = "SELECT skin FROM users_settings WHERE uid = :uid LIMIT 1";
