...
class Model_Index {
	...
	public function Pager() {
		...
		$prev = ($this->curPage-1);
		if ($prev != 0) {
			$this->pager[0] = "<div class='pager_arr'><a class='pager' href='" . $this->registry["uri"] . $this->registry["action"] . $this->links . "/page/" . $prev . "/'>←</a></div>";
		};
		...
		for ($i=1; $i <= $numPage; $i++) {
			if ($i == $this->curPage) {
				...
				if ( ($i >= $this->curPage - $this->num ) and ($i <= $this->curPage + $this->num) ) {
					$this->pager[] = "<div class='pager_text'><a class='pager' href='" . $this->registry["uri"] . $this->registry["action"] . $this->links . "/page/" . $i . "/'>$i</a></div>";
				};
			};
		};
		...
		$next = ($this->curPage+1);
		if ($next <= $numPage) {
			$this->pager[$i+1] = "<div class='pager_arr'><a class='pager' href='" . $this->registry["uri"] . $this->registry["action"] . $this->links . "/page/" . $next . "/'>→</a></div>";
		};
