<?php
class Users_Public_Functions extends Modules_Functions {
	...
	function users_date() {
		return $this->view->render("users_date", (array("date" => $groupsSess)));
	}
			...
			if (is_array($val)) {
				if ($key != "0") {
					if(is_numeric($key)) {
						...
						if ($groups) {
							$this->tree .= "<ul><li><span class='folder'><label><input type='checkbox' name='gruser[]' id='hg" . $key . "' class='gruser' title='" . $gid . "' value='" . $key . "' />&nbsp;" . $gid . "</label></span>";
						} else {
	...
	function users_tree() {
		...
		foreach($groups as $group) {
			foreach($users as $user) {
				if ( ($user["gname"] == $group["sname"]) and ($group["sname"] != null) ) {
					$udata = $this->view->render("users_lightdata", array("data" => $this->registry["user"]->getUserInfo($user["id"])));
		...
		return $this->view->render("users_lightustr", (array("group" => $uniq_groups, "list" => $this->tree)));
	}
	...
	function onlyUsers_tree() {
		...
		foreach($groups as $group) {
			foreach($users as $user) {
				if ( ($user["gname"] == $group["sname"]) and ($group["sname"] != null) ) {
					$udata = $this->view->render("users_lightdata", array("data" => $this->registry["user"]->getUserInfo($user["id"]), "radio" => true));
					$sortlist[$group["pname"]][$group["sid"]][] = $udata;
				}
			}
		}
		...
		return $this->view->render("users_lightustr", (array("group" => $uniq_groups, "list" => $this->tree, "notcheckall" => true)));
	}
