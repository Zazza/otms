<?php
class Model_TtMail extends Modules_Model {
	function addTask($remote_id, $post) {
		if ($post["task"] != '') {
			...
			$res = $this->registry['db']->prepare($sql);
			$param = array(":remote_id" => $remote_id, ":oid" => '0', ":who" => $this->registry["user"]->tid, ":imp" => $post["imp"], ":secure" => $secure, ":text" => $post["task"], ":gid" => $post["ttgid"]);
			$res->execute($param);
	...
	function addComment($remote_id, $text, $status) {
		...
		if (count($tid) == 1) {
			...
			if ($status != 0) {
				$status_text = $this->registry["tt"]->getCommentStatusText($status);
