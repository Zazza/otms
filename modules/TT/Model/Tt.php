...
class Model_Tt extends Modules_Model {
	...
	public function getTasks() {
		...
		if ($sort == "group") {
			...
			if ($id == 'false') {
				$where = "";
				$sql_inc = "ORDER BY t.oid DESC, t.imp DESC, t.id DESC";
			} else {
				$where = "AND t.oid = " . $id;
				$sql_inc = "ORDER BY t.oid DESC, t.imp DESC, t.id DESC";
			}
	...
	public function getTask($tid) {
		...
		$sql = "SELECT DISTINCT(t.id), t.oid, t.who, t.remote_id, t.mail_id, t.imp, t.secure, t.text, t.opening AS start, t.ending, t.gid, t.close, g.name AS `group`, t.cuid AS cuid, tr.uid, tr.gid AS rgid, tr.all AS `all`, td.type, td.opening, td.deadline, td.iteration, td.timetype_iteration, ts.id AS spam
	        FROM troubles AS t
	        LEFT JOIN troubles_responsible AS tr ON (tr.tid = t.id)
	...
	public function addTask($oid, $post, $mid = false) {
		...
		if ($post["task"] != '') {
			...
			if ($mid) {
				$sql = "INSERT INTO troubles (remote_id, oid, who, imp, secure, mail_id, gid) VALUES (:remote_id, :oid, :who, :imp, :secure, :mail_id, :gid)";

				$res = $this->registry['db']->prepare($sql);
				$param = array(":remote_id" => '0', ":oid" => $oid, ":who" => $this->uid, ":imp" => $post["imp"], ":secure" => $secure, ":mail_id" => $mid, ":gid" => $post["ttgid"]);
				$res->execute($param);
			} else {
				$sql = "INSERT INTO troubles (remote_id, oid, who, imp, secure, text, gid) VALUES (:remote_id, :oid, :who, :imp, :secure, :text, :gid)";

				$res = $this->registry['db']->prepare($sql);
				$param = array(":remote_id" => '0', ":oid" => $oid, ":who" => $this->uid, ":imp" => $post["imp"], ":secure" => $secure, ":text" => $post["task"], ":gid" => $post["ttgid"]);
				$res->execute($param);
			}
			...
			$string = "Новая задача <a href='" . $this->registry["uri"] . "tt/" . $tid . "/'>" . $tid . "</a>";
			if (!is_numeric($mid)) {
	...
	public function diffTask($post, $tid) {
		...
		foreach($del_uid as $part) {
			if ($part != 0) {
				$i++;
				$user = $this->registry["module_users"]->getUserInfo($part);
		...
		foreach($del_gid as $part) {
			if ($part != 0) {
				$i++;
				$group = $this->registry["module_users"]->getGroupName($part);
		...
		if ($oldTask[0]["who"] != $newTask[0]["who"]) {
			$old_author = $this->registry["module_users"]->getUserInfo($oldTask[0]["who"]);
			$new_author = $this->registry["module_users"]->getUserInfo($newTask[0]["who"]);
		...
		$string = "Правка задачи <a href='" . $this->registry["uri"] . "tt/" . $tid . "/'>" . $tid . "</a>";
	...
	public function getComments($tid) {
		...
		for($i=0; $i<count($data); $i++) {
			...
			$data[$i]["timestamp"] = $this->editDate($data[$i]["timestamp"]);
	...
	public function getNumTasks() {
		if (isset($this->registry["module_users"])) {
			...
			$gid = $user->getGidFromUid($this->registry["ui"]["id"]);
		} else {
			$gid = 1;
		}
		...
		$res = $this->registry['db']->prepare($sql);
		$param = array(":uid" => $this->registry["ui"]["id"], ":gid" => $gid);
		$res->execute($param);
		$data = $res->fetchAll(PDO::FETCH_ASSOC);
	...
	public function issetTaskFromMail($from) {
		...
		$sql = "SELECT t.id
		FROM mail_contacts AS mc
		LEFT JOIN troubles AS t ON (t.oid = mc.oid)
	...
	public function closeTask($tid, $uid = null) {
		...
		if ($close[0]["close"] == 0) {
			...
			$task = $this->getTask($tid);
			$tinfo = array();
			$string = "Завершена задача <a href='" . $this->registry["uri"] . "tt/" . $tid . "/'>" . $tid . "</a>";
	...
	public function addDraft($oid, $post) {
		...
		$sql = "INSERT INTO draft (oid, who, imp, secure, text, gid) VALUES (:oid, :who, :imp, :secure, :text, :gid)";
	...
	public function getDraft($tid) {
		...
		$sql = "SELECT t.id, t.oid, t.who, t.imp, t.secure, t.text, t.opening AS start, t.gid, g.name AS `group`, tr.uid, tr.gid AS rgid, tr.all AS `all`, td.type, td.opening, td.deadline, td.iteration, td.timetype_iteration
        FROM draft AS t
	...
	public function editDraft($post) {
		...
		$sql = "UPDATE draft SET imp = :imp, secure = :secure, text = :text, gid = :gid WHERE id = :tid LIMIT 1";
