...
class Model_Template extends Modules_Model {
    public function addTree($id,  $name) {
		$sql = "INSERT INTO templates_type (tid, name) VALUES (:tid, :name)";
		
		$res = $this->registry['db']->prepare($sql);
		$param = array(":tid" => $id, ":name" => $name);
		$res->execute($param);
    }
    ...
    public function getTidFromTtid($id) {
    	$sql = "SELECT tt.tid, tt.name
    	        FROM templates_type AS tt
    	        WHERE tt.id = :id
    	        ORDER BY tt.name";
            ...
            if (isset($post["field"])) {
                if (count($post["field"]) > 0) {
                    for($i=0; $i<count($post["field"]); $i++) {
                        ...
                        $res = $this->registry['db']->prepare($sql);
                        $param = array(":tid" => $tid, ":field" => htmlspecialchars($post["field"][$i]), ":main" => $post["main"][$i], ":expand" => $post["expand"][$i]);
                        $res->execute($param);
                    }
                }
            }
